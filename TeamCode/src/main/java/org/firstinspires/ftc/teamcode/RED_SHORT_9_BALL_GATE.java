...
@Autonomous(name = "RED_SHORT_9_BALL_GATE",group = "RED")
public class RED_SHORT_9_BALL_GATE extends OpMode {
    ...
    public enum PathState{
        ...
        DRIVE_COLLECT1,
        DRIVE_COLLECT1_TO_PRE_GATE,
        DRIVE_PRE_GATE_TO_GATE,
        DRIVE_GATE_TO_SHOOT,
        REVERSE_BALL_1,
        ...
        DRIVE_COLLECT2,
        DRIVE_COLLECT2_TO_PRE_GATE,
        DRIVE_PRE_GATE_TO_GATE2,
        DRIVE_GATE_TO_SHOOT2,
        REVERSE_BALL_2,
    ...
    private final Pose startPose = new Pose(122.84112149532712,122.39252336448601,Math.toRadians(37));
    ...
    private final Pose preGate = new Pose(113.94579439252337,74.5,Math.toRadians(0));
    private final Pose gate = new Pose(126,74.5,Math.toRadians(0));
    private final Pose preBallCollect2 = new Pose(94.36448598130843,59,Math.toRadians(0));
    private final Pose ballCollect2 = new Pose(136.54485981308412,59,Math.toRadians(0));
    ...
    public void buildPaths(){
        ...
        drivePreToBallCollect1 = follower.pathBuilder()
                ...
                .build();
        driveCollect1toPreGate = follower.pathBuilder()
                .addPath(new BezierLine(ballCollect1,preGate))
                .setLinearHeadingInterpolation(ballCollect1.getHeading(), preGate.getHeading())
                .build();
        drivePreGateToGate = follower.pathBuilder()
                .addPath(new BezierLine(preGate,gate))
                .setLinearHeadingInterpolation(preGate.getHeading(), gate.getHeading())
                .build();
        driveGateToShoot = follower.pathBuilder()
                .addPath(new BezierLine(gate,shootPose))
                .setLinearHeadingInterpolation(gate.getHeading(), shootPose.getHeading(),0.8)
                .build();
        ...
        drivePreToBallCollect2 = follower.pathBuilder()
                ...
                .build();
        driveCollect2toPreGate = follower.pathBuilder()
                .addPath(new BezierLine(ballCollect2,preGate))
                .setLinearHeadingInterpolation(ballCollect2.getHeading(), preGate.getHeading())
                .build();
        drivePreGateToGate2 = follower.pathBuilder()
                .addPath(new BezierLine(preGate,gate))
                .setLinearHeadingInterpolation(preGate.getHeading(), gate.getHeading())
                .build();
        ...
        switch (pathState){
            ...
            case DRIVE_COLLECT1:
                if (!follower.isBusy()){
                    ...
                    setPathState(PathState.DRIVE_COLLECT1_TO_PRE_GATE);
                }
                break;
            case DRIVE_COLLECT1_TO_PRE_GATE:
                if (!follower.isBusy() || pathTimer.getElapsedTimeSeconds() > 2){
                    follower.setMaxPower(1);
                    intake.setPower(0);
                    follower.followPath(driveCollect1toPreGate,true);
                    setPathState(PathState.DRIVE_PRE_GATE_TO_GATE);
                }
                break;
            case DRIVE_PRE_GATE_TO_GATE:
                if (!follower.isBusy()){
                    follower.setMaxPower(0.6);
                    follower.followPath(drivePreGateToGate);
            ...
            case DRIVE_COLLECT2:
                if (!follower.isBusy()){
                    ...
                    setPathState(PathState.DRIVE_COLLECT2_TO_PRE_GATE);
                }
                break;
            case DRIVE_COLLECT2_TO_PRE_GATE:
                if (!follower.isBusy() || pathTimer.getElapsedTimeSeconds() > 2.4){
                    follower.setMaxPower(1);
                    intake.setPower(0);
                    follower.followPath(driveCollect2toPreGate,true);
                    setPathState(PathState.DRIVE_PRE_GATE_TO_GATE2);
                }
                break;
            case DRIVE_PRE_GATE_TO_GATE2:
                if (!follower.isBusy()){
                    follower.setMaxPower(0.6);
                    follower.followPath(drivePreGateToGate2);
                    setPathState(PathState.DRIVE_GATE_TO_SHOOT2);
                }
                break;
            case DRIVE_GATE_TO_SHOOT2:
                if (!follower.isBusy() || pathTimer.getElapsedTimeSeconds() > 1.4){
                    follower.setMaxPower(1);
                    follower.followPath(driveGateToShoot2,true);
